...
candidates survive and a model endpoint is configured, it adjudicates BETWEEN the
...
def propose_patch(plan: dict, failed_step_id: int, dom_controls: list[dict]) -> dict | None:
    ...
    pick = pool[0]
    if len(pool) > 1 and llm.available():
        try:
            choice = llm.chat_json(
                f"A UI control was renamed. Original: label {old.label!r}, role {old.tag}, "
                f"region {old.region}, purpose: {step['why']}\n"
                f"Candidates (unregistered controls found in the live DOM):\n"
                f"{json.dumps(pool)}\n"
