...
**Consequence:** The `RunloopSandbox` adapter is stubbed key-ready in `kerb/engine/executor.py`; the demo tech-reveal narration must disclose "local sandbox" honestly until a key is added. All Sentinel/heal behavior is identical either way.

## D-002 · 2026-08-29 · Plan compiler: model-first with deterministic fallback
**Decision:** `compile_plan` tries the configured model endpoint (JSON-constrained) first and falls back to a deterministic compiler on any error/timeout; every plan is tagged `compiler: "model" or "deterministic"`.
...
**Decision:** `SandboxExecutor` spawns `scripts/replay_cli.py` as its own OS process (with `PYTHONDONTWRITEBYTECODE=1`), and receipt screenshots write to `~/.kerb/receipts`, never into the repo.
...
**Decision:** Repair patches live in `~/.kerb/registry_patches.json`, loaded on every `registry()` call.
