# Kerb - System Specification

The implemented contract: schemas, boundaries, and the rules every layer enforces.
Architecture narrative lives in [ARCHITECTURE.md](ARCHITECTURE.md); this document is the reference.
 ...
 policy docs ──►  RAG ──► PLAN COMPILER ──► VALIDATOR ──► PLAN STORE
             ...
             REPAIR AGENT ◄── red replays ◄─────────│ Autopilot(execute)  │
                    ...
                    └── green re-replay gate        └─────────────────────┘
                         (the sandbox provider devboxes)
```

Repo layout (already scaffolded):

```
kerb/                 Reflex app: shell, dial, renderers, overlay, command bar
...
### §6.5 Escalation card
Fires on: required step below grounding floor, policy-forbidden goal (claim #5150), or unhealable sabotage. Contents: what was attempted, the partial plan, the exact blocker (named rule or missing evidence), and a handoff summary. This is both a product feature and a safe landing for any request the system cannot serve.

### §6.6 Sabotage panel (operator-facing)
...
- **§7.1 Snapshot blueprint** (built once in Phase A): Python + Meridian in headless mode (`MERIDIAN_HEADLESS=1`: engine + app server, no compile calls — plans arrive as JSON) + Playwright + chromium + seeded DB. No model credentials inside sandboxes; they execute, they never think.
...
- **§8.1 Layout:** Meridian is the full canvas. Kerb = slim top bar (wordmark · dial · plan id · mode badge) + ⌘K command bar + right rail that exists only when a plan is active. A layer over software, not a website. No chat surface exists.
...
- **§9.2** The policy beat uses the #4821 vs #3377 plan-length contrast if asked "how do I know it read the docs?"
- **§9.3** Live sabotage: rename/move → red → patch-with-evidence → green re-replay → live spotlight on the renamed control (≤ 60s, rehearsed ≥ 10×). A deleted control or impossible request → escalation card beat, also rehearsed.
...
## §10. Build sequence and acceptance criteria

|---|---|---|
| **G0** (before all else) | the sandbox provider spike first | §7.4 rubric passes, or fallback consciously invoked and logged |
| **G1** (mid-build) | Spine first | Unedited video: an external rename → red → patch → green → live re-run |
| **G2** (end Phase D) | Honest confidence | Forced-confirm beat fires from real grounding numbers; banned vocabulary absent from UI |
| **G3** (before demo) | Externally-driven sabotage | 10 consecutive clean rehearsals incl. 2 escalation-path runs; heal ≤ 60s |
